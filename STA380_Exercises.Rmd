...
Link to the GitHub: https://github.com/VarunKausika/STA-380-Exercises
...
```{r echo=FALSE}
knitr::kable(round(PCAwine$rotation[,1:2],2), caption="Weights on the components")
```
```{r echo=FALSE}
...
pc1_summary = loadings_summary %>%
  select(Properties, PC1) %>%
  arrange(desc(PC1)) %>%
  mutate(PC1 = round(PC1,2))

knitr::kable(pc1_summary, caption="Summary of PC1")
```
...
pc2_summary = loadings_summary %>%
  select(Properties, PC2) %>%
  arrange(desc(PC2)) %>%
  mutate(PC2 = round(PC2,2))

knitr::kable(pc2_summary, caption="Summary of PC2")
```
**PC2 gives more positive loading to density and high negative to alcohol content**
...
```{r plot19, fig.cap="PC space, i.e. the space of summary variables we've created", fig.height=3, echo=FALSE}
wine %>% ggplot(aes(x = PC1, y = PC2)) + geom_point()

```

\begin{itemize}
\item We can see that there are two clusters emerging. One to the left having more negative PC1 and another to the right having more positive PC1.
\item As we do not have access to the wine color, we cannot say exactly which of these two clusters correspond to red and White wine.
\item However, the difference in the loadings for each feature by the principal components implies that these two clusters are distinct.
\end{itemize}
 
\subsection{Categorize quality}
...
knitr::kable(table(wine$Q_score), caption="Quality")
```

```{r plot20, fig.cap="It doesn't look like the principal components can do a good job of separating high and low quality wines", fig.height=3, echo=FALSE}
...
knitr::kable(clust1$center[1,]*sigma + mu, caption="Cluster center 1")
```
```{r, warning=FALSE}
knitr::kable(clust1$center[2,]*sigma + mu, caption="Cluster center 2")
```


**Observations**
\begin{itemize}
\item More sulfur dioxide and sugar compared to cluster 2.
\item More acidity, chlorides and sulphates compared to cluster 1.
\end{itemize}

\subsection{How close the clusters are to each other}
```{r echo=FALSE}
knitr::kable(sort(clust1$center[1,] - clust1$center[2,]), caption="Cluster center-1 to cluster center-2")
```
This shows that the cluster centers are quite close to each other.

\subsection{Table clusters}

```{r echo=FALSE}
knitr::kable(table(clust1$cluster, wine$color), caption="Confusion Matrix")
```
```{r echo=FALSE}
...
knitr::kable(head(select(wine, c(dist1, dist2))), caption="First 10 rows of PCs for wine data")

```

\subsection{Visualization}

\subsubsection{Contrasting features}
```{r plot21, fig.cap="Using the two most contrasting features for the 2 clusters as x and y to visualize", fig.height=3, echo=FALSE}
...
\subsubsection{Plot for High and Low Quality Wine}
```{r plot22, fig.cap="We can see that the features are not very discriminative of quality.", fig.height=3, echo=FALSE}
wine$Q_score <- NA
wine[wine$quality <= 5,]$Q_score <- 'Low Quality'
wine[wine$quality > 5,]$Q_score <- 'High Quality'
...
```{r plot23, echo=FALSE, fig.cap="Interaction of red and white wine variables individually in the plot", fig.height=3}
wine %>% ggplot(aes(x = dist1, y = dist2, col = color)) +
  geom_point() +
  facet_grid(cols = vars(color))

```
```{r plot24, fig.cap="Interaction of red and white wine variables in the plot", fig.height=3}
wine %>% ggplot(aes(x = dist1, y = dist2, col = color)) +
  geom_point()

```

```{r plot25, echo=FALSE, fig.cap="Interaction of red and white wine variables wrt quality parameter in the plot.", fig.height=3}
...
\begin{itemize}
\item PCA is the best dimensionality reduction technique makes more sense to us for this data.
\item PCA Counteracts the issues of high dimensional data.
\item PCA improves performance at a very low cost of model accuracy.
\end{itemize}
...
```{r plot27, fig.cap="The plot is showing us that college uni and online gaming are correlated. We also see that health nutrition and personal fitness are correlated as well."}
...
```{r plot28, results='hide', fig.cap="Looking at the 10th principal component we can see that roughly 64.6 percent of the fraction of variance is explained. Using the Kaiser Criterion we can drop all Eigen values under 1.0.", fig.height=3}
...
```{r plot29, fig.cap="Visualization of the clusters. Using the elbow method we've determine the number of clusters. We've decided to move forward with 10 clusters.", fig.height=4}
set.seed(123)
...
```
\subsection{Summary of the principal components}
```{r}
...
```{r plot30, fig.cap="Cluster 1 primarily composed with people who have interest in current events, traveling and politics.", fig.height=3}
...
```{r plot31, fig.cap="Cluster 2 primarily composed with people who have interest in shopping, current events, college uni.", fig.height=3}
#Cluster 2 
...
```{r plot32, fig.cap="Cluster 3 emphasized people who have interest in dating then followed by fashion.", fig.height=3}
#Cluster 3
...
```{r plot33, fig.cap="Cluster 4 primarily composed of people who have interest in sports fandom, religion, and parenting.", fig.height=3}
#Cluster 4
...
```{r plot34, fig.cap="Cluster 5 primarily composed of people who have interest in cooking, fashion and beauty. ", fig.height=3}
#Cluster 5
...
```{r plot35, fig.cap="Cluster 6 primarily composed of people who have interest in college unit, online gaming and sports playing.", fig.height=3}
#Cluster 6
...
```{r plot36, fig.cap="Cluster 7 primarily composed of people who have interest in art, tv film, and current events.", fig.height=3}
#Cluster 7
...
```{r plot37, fig.cap="Cluster 8 primarily composed of people who have interest in news, politics, and automotive.", fig.height=3}
#Cluster 8
...
```{r plot38, fig.cap="Cluster 9 primarily composed of people who have interest in politics, travel, computers.", fig.height=3}
#Cluster 9
...
```{r plot39, fig.cap="Cluster 10 primarily composed of people who have interest in personal fitness, cooking, outdoors.", fig.height=3}
#Cluster 10
...
```
\begin{enumerate}
\item What question are we trying to answer - What topics does Bernard Hickey write about?
\item What statistical tool did we use to answer the question - We calculated tf-idf scores of each of the words in the document, then used those scores to perform a PCA and then clustered the documents based on similar topics and visulized it in a dendrogram.
\item Results: What evidence/results did your approach provide to answer the questions? 
\begin{itemize}
\item We obtained 5 major clusters and categorized all of Bernard's documents in those clusters. 
\item Then we looked at the documents in each cluster and found what topics each cluster represents.
\end{itemize}
\item Conclusion: What are your conclusions about your questions? Provide a written interpretation of your results, understandable to stakeholders who might plausibly take an interest in this data set. - Bernard mostly writes about financials of companies, mergers and acquisitions, business and investing, politics and media.
\end{enumerate}
 
\subsection{Preprocessing steps}
1. Created a reader function to read documents.
2. Take out the second part of the file name and rename the files.
3. Create a corpus.
4. pre-processing/tokenization steps.
5. Removing stopwords.
6. Creating a doc-term-matrix.
7. Inspecting its entries.
...
```
\subsection{Basic summary statistics of documents}
```{r}
...
```
\subsection{Finding frequently used terms}
```{r}
...
1. Drop those terms that only occur in one or two documents.
2. Construct tf-idf weights.
...
\subsection{Comparing Documents}

Here, we observe term weights in the first document.
```{r}
inspect(tfidf_bernard[1,])
```

Now, we compare this with the actual content of the first document.
```{r}
...
We performed PCA on the tf-idf scores.
```{r plot40, fig.cap="The first 2 principal components seem to retain most of the variance of the data.", fig.height=3}
...
```
We filter our dataset to represent each document with just 2 numbers from PCA.
```{r plot41, fig.cap="There are 5 clear clusters, most of the documents seem to fall in the third.", results='hide', fig.height=3}
pca_bernard$x[,1:2]
...
```
\subsection{Comparing documents in clusters}

**"Boosting revenue" topic - document 48 and 50**
```{r}
knitr::kable(content(bernard[[48]]), caption="Document 48")
knitr::kable(content(bernard[[50]]), caption="Document 50")
```

**"Mergers/Acquisitions" topic - document 28 and 29**
```{r}
knitr::kable(content(bernard[[28]]), caption="Document 28")
knitr::kable(content(bernard[[29]]), caption="Document 29")
```

**"Australia-Murdoch media coverage" topic - documents 9 and 10**
```{r}
...
\subsection{Cluster documents}
Here we use hierarchical clustering to plot a dendrogram according to the PCA scores calculated earlier.

```{r plot42, fig.cap="Document 7 doesn't seem to relate to any of the other documents.", fig.height=3}
# pca_bernard$x
...
```
\subsection{Inspect the clusters}
Both documents belong to the Merger topic.
```{r, results=FALSE}
which(clust5 == 3)
...
\section{9. Association Rules Mining}

\subsection{Approach}
\begin{enumerate}
\item Before finding interesting association rules, we plotted the frequency of the top 10 items in all baskets. We can see that whole milk, other vegetables, and rolls/buns are the most frequent items in all baskets. For the apriori algorithm, we used a support of 0.005, confidence of 0.25 and then examined those that had a lift greater than 3. The support and confidence were chosen through trial and error of seeing the number of rules different values result in. A lift of 3 was chosen in order to only examine rules that lead to a higher probability of occurrence. After obtaining the rules using these thresholds, we visualized them in the network shown below. The areas of concentrated blue where whole milk appears to be the central node to a variety of vegetables and fruits is representative of the fact that these grocery items are frequently purchased together and with many other items (other nodes).
\item To look further into this, we sorted the rules by confidence and lift separately. We found that any time vegetables, fruit, and dairy products are purchased, the lift for whole milk was greater than 2.5. This means that the probability someone purchases whole milk increases if they are purchasing fresh produce and dairy products. We also see that the probability of purchasing vegetables increases if someone is already purchasing vegetables of a different kind. With respect to all dairy products, the probability of purchasing them increases when someone is purchasing fruit, which could be indicative of people making fruit smoothies or putting fruits in their yogurt.
\item Overall, the top 30 rules sorted by confidence and lift make sense since these groceries are considered essential to cook any meal and some are even complementary goods (i.e. fruit and yogurt, rolls/buns and sausage). Additionally, these rules can be used by grocery stores to increase sales by placing these products near each other. In fact, they are probably already using these rules to their advantage!
\end{enumerate}

```{r grocery, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Plot of the item frequency of groceries in all baskets. Whole milk, other vegetables, and rolls/buns are the most frequently occuring items in baskets."}
library(igraph)
library(arules)
library(arulesViz)
...
```{r apri, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r all, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Network of grocery rules that have a support of 0.005, confidence of 0.25, and lift greater than 3. Concentration of blue coloring near whole milk and various fruit and vegetables indicates a co-occurence pattern.", fig.height=2}
...
```{r conf, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Network of the top 30 grocery rules sorted according to confidence. Displays the idea that the probability of purchasing whole milk increases with the purchase of fruit and vegetables.", fig.height=2}
